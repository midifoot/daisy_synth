...
int main(void) {
    ...
    // ==========================================
    // 2. BUTTONS INIT (NEW MAPPING)
    // ==========================================
    daisy::GPIO G_button;
    daisy::GPIO R_button;
    G_button.Init(daisy::seed::D7, daisy::GPIO::Mode::INPUT, daisy::GPIO::Pull::PULLUP); // Physical Pin 8
    R_button.Init(daisy::seed::D8, daisy::GPIO::Mode::INPUT, daisy::GPIO::Pull::PULLUP); // Physical Pin 9

    // ==========================================
    // 3. ENCODERS INIT (NEW MAPPING)
    // ==========================================
    daisy::Encoder enc1;
    daisy::Encoder enc2;
    daisy::Encoder enc3;
    // Format: Init(Phase A, Phase B, Click)
    enc1.Init(seed::D20, seed::D19, seed::D21); // Physical 26, 27, 28 (inversion on software)
    enc2.Init(seed::D23, seed::D22, seed::D24); // Physical 29, 30, 31 (same)
    enc3.Init(seed::D26, seed::D25, seed::D27); // Physical 32, 33, 34 (same)

    // ==========================================
    // 4. RGB LED INIT (UNCHANGED)
    // ==========================================
    daisy::RgbLed rgb;
    ...
    while(1) {
        ...
        // ==========================================
        // UPDATE RGB LED HARDWARE
        // ==========================================
        rgb.Set(r_val, g_val, b_val);
        rgb.Update(); 
        ...
        if (needsDisplayUpdate) {
            ...
            display.WriteString(valStr, Font_5x8, !isVolSelected); 
