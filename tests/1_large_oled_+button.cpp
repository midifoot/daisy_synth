...
int main(void) {
    ...
    // Initialize the buttons on pin D0 & D1
    daisy::GPIO G_button;
    daisy::GPIO R_button;
...
while(1) {
        ...
        // Because we use PULLUP to GND, the pin reads 'false' (0) when pressed.
        // We use the '!' (NOT) operator so isPressed is 'true' when you push it.
        ...
        // 1. MIDI Indicator (Small Circle) - Now reactive!
        if (GBPressed || RBPressed) {
            // Draw a solid filled circle if button is held
            display.DrawCircle(4, 4, 3, true); 
            display.DrawCircle(4, 4, 2, true);

        } else {
            // Draw nothing if buttons are released 
            //
        }
        // static button (old version)
        //display.DrawCircle(4, 4, 3, true); 
        //display.DrawCircle(4, 4, 2, true);
